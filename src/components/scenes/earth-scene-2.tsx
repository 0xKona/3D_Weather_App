...
const EarthModel = ({ coords, onLocationSelect, isUserInteracting = false }: Props) => {
  ...
  useEffect(() => {
    if (earthGroupRef.current && coords && coords[0] && coords[1]) {
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        ...
        const finalRotationY = baseRotationY - lngRad;
        const finalRotationX = -latRad;
  ...
  useFrame((state) => {
    ...
    // Only auto-rotate if not currently animating to a location and user is not interacting
    if (earthGroupRef.current && !gsap.isTweening(earthGroupRef.current.rotation) && !isUserInteracting) {
...
const EarthScene2 = ({ coords, onLocationSelect }: Props) => {
  ...
  return (
    ...
    >
      ...
      />
      <EarthModel coords={coords} onLocationSelect={onLocationSelect} isUserInteracting={isUserInteracting} />
    </Canvas>
  );
};

export default EarthScene2;
